...
(() => {
  ...
  document.addEventListener('click', (event) => {
    const target = event.target && typeof event.target.closest === 'function' ? event.target.closest('button, a') : null;
    if (!target) return;
    if (target.matches('[data-copy]')) track('install_copy');
    else if (target.id === 'combine-button') track('playground_run');
  ...
  globalThis.gtag('config', measurementId, { allow_ad_personalization_signals: false, allow_google_signals: false, anonymize_ip: true });
  ...
  function track(action) {
    ...
    globalThis.gtag('event', action, { event_category: 'package_docs', package_name: '@stackline/combine-source-map' });
  }
})();
